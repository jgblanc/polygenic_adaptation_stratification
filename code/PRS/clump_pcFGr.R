...
if( length(args) != 4){stop("Usage: <causal effects file> <prefix for glm.linear file> <output file prefix> <List of PCs>") }
...
main_nc <- function(gwas_path, out_path) {
  ...
  if (n_snps < length(nchrms)) {n <- n_snps} else {n <- length(nchrms)}
  for (i in 2:n) {
