...
envs  <-  c("env-0.01", "env-0.2", "env-0.0")
cases <- c("C1")
test <- c("PS", "LAT")
#test <- c("PS")
pheno <- c("PS", "DIAG", "LAT")
...
agg_all_data <- function(rep, dir_path, case, h2, pheno, envs, test) {
  ...
  Qx <- fread(paste0(dir_path, rep,"/", case, "/", h2, "/", pheno, "/", envs, "/", test, "/",  "Qx_ss.txt"))
...
fwrite(df, "SimpleGrid_fp_A_SS_random.txt.gz", row.names=F,quote=F,sep="\t", col.names = T)
