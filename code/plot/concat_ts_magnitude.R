...
agg_all_data <- function(rep, dir_path, case, type, h2, env,ts) {
  ...
  if (h2  == "diff-0.3" & env == "env-0.0" ) {
    Qx <- as.data.frame(matrix(NA, nrow=1,ncol=1))
    colnames(Qx) <- c("ts_magnitude")
} else {  
  Qx <- fread(paste0(dir_path, rep,"/", case, "/", h2, "/", ts, "/",  env, "/", "ts_magnitude.txt"))
}  
  return(Qx)
}
df <- plyr::mdply(dat, agg_all_data, dir_path = '../../output/Simulate_Phenotypes/4PopSplit/' )

fwrite(df, "4PopSplit_L100_ts_magnitude.txt", row.names=F,quote=F,sep="\t", col.names = T)
