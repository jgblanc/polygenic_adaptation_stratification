...
fwrite(df, "4PopSplit_F100_fullgrid.txt", row.names=F,quote=F,sep="\t", col.names = T)

nc <- df %>% filter(type == "nc"| type == "nc-Tm") %>% filter(case == "C1" | case == "C2")  %>% group_by(env, type, case) %>% summarise(fp_strat = sum(`P-EN` < 0.05)/ n(), avg_Ax = mean(Ax))
