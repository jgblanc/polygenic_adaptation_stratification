...
if(length(args)<5){stop("Rscript Compute_TGWAS.R <test_prefix> <gwas_prefix> <tvec_file> <out_prefix> <number of SNPs to downsample>")}
...
compute_b <- function(path_to_test, path_to_gwas, path_to_testvec, outpath) {
  ...
  snpfile <- paste0(outpath, "SNPs.txt")
  cmd_XT <- paste("sh code/Calculate_Tm/compute_XT_downsample.sh", path_to_test, path_to_testvec, outfile_XT,snpfile ,sep = " ")
  system(cmd_XT)
...
# Read in SNP IDS and randomly sample
snps <- fread(paste0(gwas_prefix, ".afreq"),fill=TRUE )
snps <- sample_n(snps, numSNPs) %>% select(ID)
fwrite(snps, paste0(out_prefix, "SNPs.txt"), row.names = F, col.names = F, quote = F, sep = "\t")
