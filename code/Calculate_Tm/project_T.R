#script uses plink to compute Tm

args=commandArgs(TRUE)

if(length(args)<4){stop("Rscript project_T.R <test geno prefix> <gwas geno prefix> <tvec> <out prefix>")}
...
compute_b <- function(path_to_test, path_to_gwas, path_to_testvec, outpath) {
  ...
  cmd_XT <- paste("sh code/Calculate_Tm/compute_XTinv.sh", path_to_test, path_to_testvec, outfile_XT, sep = " ")
  system(cmd_XT)
  ...
  cmd_count <- paste("sh code/Calculate_Tm/compute_GWAScount.sh", path_to_gwas, outfile_count, sep = " ")
  system(cmd_count)
  ...
  fwrite(beta_reformat, paste0(outpath, "xt_temp.InTvec.glm.linear"), sep = "\t")

  # Compute b
  outfile_b <- paste0(outpath, "b")
  cmd_b <- paste("sh code/Calculate_Tm/GWAS_score.sh", path_to_gwas, paste0(outpath, "xt_temp.InTvec.glm.linear"), outfile_b, sep = " ")
  system(cmd_b)
