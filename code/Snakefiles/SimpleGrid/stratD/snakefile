...
# note this test vector is mean centered but not standardized
rule make_test_vector:
    input:
        pops="output/Simulate_Genotypes/SimpleGrid/{rep}/genos.pop",
        fam="output/Simulate_Genotypes/SimpleGrid/{rep}/{config}/genos-test_common.psam"
    output:
        lat="output/Calculate_Tm/SimpleGrid/{rep}/{config}/Tvec.txt"
    shell:
        "Rscript code/Calculate_Tm/make_tvec_SimpleGrid.R {input.pops} {input.fam} {output.lat} {output.long}"
