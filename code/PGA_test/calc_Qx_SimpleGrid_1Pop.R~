...
out_pre_one = args[15] # output prefix
out_pre_two = args[16]
out_pre_three = args[17]
out_pgs = args[18]
#out_pre = "~/polygenic_adaptation_stratification/output/Empirical_Null/4PopSplit/E1/C1/h2-0/env-0.0/geno-gwas_"


#c_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common.c.betas"
#cp_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common.c.p.betas"
#nc_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common.nc.betas"
#c_Tm_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common-Tm.c.betas"
#cp_Tm_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common-Tm.c.p.betas"
#nc_Tm_file="output/PRS/SimpleGrid/E2/C1/h2-0/env-0.0/genos-gwas_common-Tm.nc.betas"
#geno_prefix="output/Simulate_Genotypes/SimpleGrid/E2/C1/genos-test_common"
#lambdaT_file="output/Calculate_Tm/SimpleGrid/E2/C1/Lambda_T.txt"
#Va_file="output/PGA_test/SimpleGrid/E2/C1/h2-0/env-0.0/Va.txt"
#Va_Tm_file="output/PGA_test/SimpleGrid/E2/C1/h2-0/env-0.0/Va-Tm.txt"
#true_file="output/PRS/SimpleGrid/E2/C1/h2-0/genos-test_common.true.sscore"
#tvec_file="output/Calculate_Tm/SimpleGrid/E2/C1/Tvec.txt"
#pops_file="output/Simulate_Genotypes/SimpleGrid/E2/genos.pop"
...
# Load Latitude Test vector
std.tvec <- fread(tvec_file)
tvec <- as.matrix(std.tvec$T1)
...
fwrite(out, out_pre_one,row.names=F,quote=F,sep="\t", col.names = T)

# Rerun for 2nd pop
lambda_T <- fread(lambdaT_file)
lambda_T <- as.numeric(as.character(lambda_T[2,1]))
tvec <- as.matrix(std.tvec$T2)

# Run all types of PGS for longitude
out <- matrix(NA, nrow = 6, ncol =5)
out[1, ] <- main(c_file, Va_all[1,])
out[2, ] <- main(cp_file, Va_all[2,])
out[3, ] <- main(nc_file, Va_all[3,])
...
# Save output
colnames(out) <- c("Qx", "P-Chi", "P-EN", "Ax", "P-Ax")
fwrite(out, out_pre_two,row.names=F,quote=F,sep="\t", col.names = T)

# Rerun for 3rd pop
lambda_T <- fread(lambdaT_file)
lambda_T <- as.numeric(as.character(lambda_T[3,1]))
tvec <- as.matrix(std.tvec$T3)
...
# Save output
colnames(out) <- c("Qx", "P-Chi", "P-EN", "Ax", "P-Ax")
fwrite(out, out_pre_three,row.names=F,quote=F,sep="\t", col.names = T)
