...
```{r}
tp <- fread("~/polygenic_adaptation_stratification/output/Simulate_Genotypes/4PopSplit/tp.afreq", header = T)
gp <- fread("~/polygenic_adaptation_stratification/output/Simulate_Genotypes/4PopSplit/gp.afreq", header = T)
...
write.table(dat$ID, "~/Desktop/GWAS.txt", quote = F, col.names = F, row.names = F)
write.table(test, "~/Desktop/GWAS.txt", quote = F, col.names = F)

gwas <- subset(pop, pop$V3 == "A" | pop$V3 == "B")
test <- subset(pop, pop$V3 == "C" | pop$V3 == "D")  

write.table(gwas, "~/Desktop/GWAS.txt", quote = F, col.names = F)
write.table(test, "~/Desktop/GWAS.txt", quote = F, col.names = F)
```

```{r}
prs <- fread("~/polygenic_adaptation_stratification/output/PRS/4PopSplit/V1/C1/genos-test_common.c.sscore")
true <- fread("~/polygenic_adaptation_stratification/output/PRS/4PopSplit/V1/C1/genos-test_common.true.sscore")
pop.info <- fread("~/polygenic_adaptation_stratification/output/Simulate_Genotypes/4PopSplit/V1/C1/ids.test", header =F)

prs$Pop <- pop.info$V3
true$Pop <- pop.info$V3

ggplot(data = prs, aes(x=Pop,y=SCORE1_SUM)) + geom_boxplot()

ggplot(data = true, aes(x=Pop,y=SCORE1_SUM)) + geom_boxplot()
```


```{r}
prs <- fread("~/polygenic_adaptation_stratification/output/PRS/4PopSplit/V1/C2/genos-test_common.c.sscore")
true <- fread("~/polygenic_adaptation_stratification/output/PRS/4PopSplit/V1/C2/genos-test_common.true.sscore")
pop.info <- fread("~/polygenic_adaptation_stratification/output/Simulate_Genotypes/4PopSplit/V1/C2/ids.test", header =F)
...
```{r}
betas_c1 <- fread("~/polygenic_adaptation_stratification/output/Simulate_Phenotypes/4PopSplit/V1/C1/genos-gwas_common.effects.txt")

betas_c2 <- fread("~/polygenic_adaptation_stratification/output/Simulate_Phenotypes/4PopSplit/V1/C2/genos-gwas_common.effects.txt")
```

```{r}
library(snpStats)
SNPS <- snpStats::read.plink("/Users/jenniferblanc/scratch/practice2")
TEST <- snpStats::read.plink("/Users/jenniferblanc/scratch/practice2")

X <- as(SNPS$genotypes,"numeric")
M <- as(TEST$genotypes, "numeric")
