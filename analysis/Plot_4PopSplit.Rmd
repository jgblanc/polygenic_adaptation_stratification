...
reps <- list("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9")
...
# Histogram 
ggplot(data = prs, aes(x=RANDOM, fill = POP)) + geom_histogram(data = subset(prs, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("PRS Random") + scale_fill_manual(values = cols_test)
```

Polygenic Score in Test Population (estimated effects at known causal sites) - Stratified Phenotype
```{r}
# Box Plots
ggplot(prs, aes(x=POP,y=STRAT, fill = POP)) + geom_boxplot() + facet_wrap(~REP) + theme_bw() + ylab("PRS Stratification") + scale_fill_manual(values = cols_test)

# Histogram 
ggplot(data = prs, aes(x=STRAT, fill = POP)) + geom_histogram(data = subset(prs, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("PRS Stratification") + scale_fill_manual(values = cols_test)
```
...
# Histogram 
ggplot(data = gv, aes(x=SCORE1_SUM, fill = POP)) + geom_histogram(data = subset(gv, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("True GV") + scale_fill_manual(values = cols_test)
```
