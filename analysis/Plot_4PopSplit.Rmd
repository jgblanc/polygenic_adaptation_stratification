...
agg_phenos <- function(rep, dir_path, case, type, h2, env) {
  
  phenos <- fread(paste0(dir_path, rep,"/", case, "/", h2, "/", env, "/", "genos-gwas_common.phenos.txt"))
  
  return(phenos)
}
df_phenos <- plyr::mdply(dat, agg_phenos, dir_path = '../output/Simulate_Phenotypes/4PopSplit/' )
```
...
df <- df %>% filter(type == "nc" & env == "env-0.0")
df_Qx <- df %>% group_by(type, rep, env) %>% summarise(fp_strat = sum(`P-EN` < 0.05)/ n())

#ggplot(data = test, aes(x = type, y = env, fill = avg_Ax)) +geom_tile()
```
