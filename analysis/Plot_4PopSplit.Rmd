...
prs <- plyr::ldply(reps, agg_test, file_path='../output/PRS/4PopSplit/', file_name_prs='/C1/h2-0/env-5/genos-test_common.nc.sscore', file_name_gv='/C1/h2-0/genos-test_common.true.sscore')
...
agg_all_cov <- function(rep, dir_path, case, type) {
  ...
  mod <- glm(std.tvec ~ mprs.adj$strat.adjusted, family = "binomial")
...
```{r}
plot_cov <- df %>% group_by(TYPE) %>% summarise(STRAT = mean(COV_STRAT), RANDOM = mean(COV_RANDOM)) %>% gather(PHENO, PRS, -TYPE)
