...
reps <- list("B1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9")
...
PRS - True GV ($X\hat\beta-X \beta$) Random
```{r}
df <- as.data.frame(cbind((prs$RANDOM - gv$SCORE1_SUM), (prs$STRAT- gv$SCORE1_SUM), prs$REP, prs$POP))
colnames(df) <- c("Random", "Strat", "REP", "POP")
df$Random <- as.numeric(levels(df$Random))[df$Random]
df$Strat <- as.numeric(levels(df$Strat))[df$Strat]

# Box Plots
ggplot(df, aes(x=POP,y=Random, fill = POP)) + geom_boxplot() + facet_wrap(~REP) + theme_bw() + ylab("True GV") + scale_fill_manual(values = cols_test)

# Histogram 
ggplot(data = df, aes(x=Random, fill = POP)) + geom_histogram(data = subset(df, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("True GV") + scale_fill_manual(values = cols_test)
```

PRS - True GV ($X\hat\beta-X \beta$) Stratified
```{r}
# Box Plots
ggplot(df, aes(x=POP,y=Strat, fill = POP)) + geom_boxplot() + facet_wrap(~REP) + theme_bw() + ylab("True GV") + scale_fill_manual(values = cols_test)

# Histogram 
ggplot(data = df, aes(x=Strat, fill = POP)) + geom_histogram(data = subset(df, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("True GV") + scale_fill_manual(values = cols_test)
```
...
prs <- plyr::ldply(reps, agg_test, file_path='../output/PRS/4PopSplit/', file_name='/C2/genos-test_common.c.sscore')
cols_test <- c("darkgoldenrod1", "darkorchid1")
...
# Box Plots
ggplot(df, aes(x=POP,y=Random, fill = POP)) + geom_boxplot() + facet_wrap(~REP) + theme_bw() + ylab("PRS - GV") + scale_fill_manual(values = cols_test)

# Histogram 
ggplot(data = df, aes(x=Random, fill = POP)) + geom_histogram(data = subset(df, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("PRS - GV") + scale_fill_manual(values = cols_test)
```

PRS - True GV ($X\hat\beta-X \beta$) Strat
```{r}
# Box Plots
ggplot(df, aes(x=POP,y=Strat, fill = POP)) + geom_boxplot() + facet_wrap(~REP) + theme_bw() + ylab("PRS - GV") + scale_fill_manual(values = cols_test)

# Histogram 
ggplot(data = df, aes(x=Strat, fill = POP)) + geom_histogram(data = subset(df, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30) + facet_wrap(~REP) + theme_bw() + xlab("PRS - GV") + scale_fill_manual(values = cols_test)
```

# Look at including first 10 PCs 

```{r}
gvalue <- fread("../output/PRS/4PopSplit/B1/C1/genos-test_common.true.sscore")
gwas.pop <- fread("../output/Simulate_Genotypes/4PopSplit/B1/C1/ids.test", header = F)
colnames(gwas.pop) <- c("#IID", "FID", "POP")
gvalue <- suppressMessages(full_join(gvalue,gwas.pop))

ggplot(data = gvalue, aes(x=SCORE1_SUM, fill = POP)) + geom_histogram(data = subset(gvalue, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(gvalue, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)
```


```{r}
prs <- fread("../output/PRS/4PopSplit/B1/C1/genos-test_common.nc.sscore")
gwas.pop <- fread("../output/Simulate_Genotypes/4PopSplit/B1/C1/ids.test", header = F)
colnames(gwas.pop) <- c("#IID", "FID", "POP")
prs <- suppressMessages(full_join(prs,gwas.pop))
colnames(prs) <- c("IID", "NAMED_ALLELE_DOSAGE_SUM", "RANDOM", "STRAT", "FID", "POP")

ggplot(data = prs, aes(x=RANDOM, fill = POP)) + geom_histogram(data = subset(prs, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)


ggplot(data = prs, aes(x=STRAT, fill = POP)) + geom_histogram(data = subset(prs, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)
```

PRS - True GV ($X\hat\beta-X \beta$) Strat
```{r}
df <- as.data.frame(cbind((prs$RANDOM - gvalue$SCORE1_SUM), (prs$STRAT- gvalue$SCORE1_SUM), prs$POP))
colnames(df) <- c("Random", "Strat", "POP")
df$Random <- as.numeric(levels(df$Random))[df$Random]
df$Strat <- as.numeric(levels(df$Strat))[df$Strat]

# Histogram 
ggplot(data = df, aes(x=Strat, fill = POP)) + geom_histogram(data = subset(df, POP == "B"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS - GV") + scale_fill_manual(values = cols_test)
```


```{r}
gvalue <- fread("../output/PRS/4PopSplit/B1/C2/genos-test_common.true.sscore")
gwas.pop <- fread("../output/Simulate_Genotypes/4PopSplit/B1/C2/ids.test", header = F)
colnames(gwas.pop) <- c("#IID", "FID", "POP")
gvalue <- suppressMessages(full_join(gvalue,gwas.pop))

ggplot(data = gvalue, aes(x=SCORE1_SUM, fill = POP)) + geom_histogram(data = subset(gvalue, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(gvalue, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)
```

```{r}
tB <- fread("../output/Simulate_Phenotypes/4PopSplit/B1/C1/genos-gwas_common.effects.txt")
eB <- fread("../output/PRS/4PopSplit/B1/C1/genos-gwas_common.c.betas")

plot(tB$V3, eB$BETA_Random)
abline(a = 0, b =1)

plot(tB$V3, eB$BETA_Strat)
abline(a = 0, b =1)
```


```{r}
prs <- fread("../output/PRS/4PopSplit/B1/C2/genos-test_common.c.sscore")
gwas.pop <- fread("../output/Simulate_Genotypes/4PopSplit/B1/C2/ids.test", header = F)
colnames(gwas.pop) <- c("#IID", "FID", "POP")
prs <- suppressMessages(full_join(prs,gwas.pop))
colnames(prs) <- c("IID", "NAMED_ALLELE_DOSAGE_SUM", "RANDOM", "STRAT", "FID", "POP")

ggplot(data = prs, aes(x=RANDOM, fill = POP)) + geom_histogram(data = subset(prs, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)


ggplot(data = prs, aes(x=STRAT, fill = POP)) + geom_histogram(data = subset(prs, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(prs, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS") + scale_fill_manual(values = cols_test)
```
...
# Histogram 
ggplot(data = df, aes(x=Strat, fill = POP)) + geom_histogram(data = subset(df, POP == "C"), alpha = 0.75, bins = 30) + geom_histogram(data = subset(df, POP == "D"), alpha = 0.75, bins = 30)  + theme_bw() + xlab("PRS - GV") + scale_fill_manual(values = cols_test)
```
