...
```{r}
df0 <- subset(df, df$env == "env-0.0") 
df0 <- df0 %>% group_by(rep) %>% mutate(nc_stan = (nc - mean(nc)) / sd(nc), nc_Tm_stan = (nc_Tm - mean(nc_Tm)) / sd(nc_Tm)) %>% group_by(LAT, LONG) %>% summarise(avg_PRS = mean(nc_stan), avg_PRS_Tm = mean(nc_Tm_stan), c = n())

ggplot(df0, aes(x = LONG, y = LAT, fill = avg_PRS)) + geom_tile()  + theme_classic() +  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) + scale_fill_gradient2(low = "red", high = "blue", mid = "white", limits = c(-1,1))
