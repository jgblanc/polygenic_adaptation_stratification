...
#axis(side=1, x, labels=formatC(x,  big.mark=','))
text(x=670000, y=0.1, labels="300,000", col = "red")
legend("topleft", inset = 0.05,legend = Fts, col=col, lty=1, title=TeX("$f_{TX}$"),bg="light grey")
dev.off()
```
...
```{r}
df <- fread("../code/Make_Plots/A_4PopSplit_joint_effects.txt.gz")
df$L2 <- df$L^2

df_plot <- df %>% filter(envs == "env_0.0", type %in% c("uncorrected_ascertained")) %>% group_by(L, L2, type) %>% summarise(var_marg = var(q_marginal), var_joint = var(q_joint), var_Va_marg = var(Va_marginal), var_Va_joint = var(Va_joint))
...
ggplot(data = df2, aes(x = L, y = value, color = variable)) + geom_point() + geom_smooth(method = "lm") + ylab("Var(q)")
```
