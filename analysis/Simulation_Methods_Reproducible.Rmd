...
fp <- fread("../code/Make_Plots/SimpleGrid_fp_C_new.txt.gz")
...
```
```{r}
dat <- gwas_weights %>% filter(test == "PS", rep == "C1")
#%>% group_by(PC) %>% summarise(relative_avg_weight = mean(Relative_PC_weights))
  
ggplot(data = dat, aes(x=PC, y=Relative_PC_weights)) + geom_col(color = "purple4") + theme_classic(base_size = 12) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = "none") + xlab("Principal Component") + ylab("Avg. Relative Weight") 
```
...
ggsave("~/Desktop/test.png", out, height = 10, width = 12)
```
