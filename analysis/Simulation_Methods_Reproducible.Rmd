...
dat  <- df %>% dplyr::group_by( case, envs, nc, type) %>% summarise(avg_marg = mean(q_marginal), avg_joint = mean(q_joint)) %>% separate(envs, c("tmp", "diff"), "_") %>% separate(nc, c("tmp2", "L"), "-") %>% filter(case == "C1", type %in% c( "nc-uncorrected")) %>% select(diff, L, avg_marg, avg_joint, type)

df_plot <- reshape2::melt(dat)

ggplot(data =df_plot, aes(x = diff, y = value, color = variable, shape = L)) + geom_point()
```
