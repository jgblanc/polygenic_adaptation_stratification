...
dat <- df %>% filter(case == "C1") %>% group_by(envs, type, case) %>% summarise(avg_q = mean(q_joint), count = n()) %>% filter(type %in% c("nc-uncorrected", "nc-Tm")) %>% separate(envs, c("env", "diff"), "_")%>% mutate(diff = as.numeric(diff))

ggplot(data = dat, aes(x = diff, y =avg_q, color = type)) + geom_point()
```
...
sv1 <- ggpubr::ggarrange(pl_phenotype, pl_pgs, pl_fp, pl_gw, nrow = 1, widths = c(0.2, 0.48, 0.2, 0.22))
sv1
...
sv2 <- ggpubr::ggarrange(pl_phenotype, pl_pgs, pl_fp, pl_gw, nrow = 1, widths = c(0.2, 0.48, 0.2, 0.22))
sv2
#sv <- ggarrange(pl_phenotype, pl_pgs, pl_fp, nrow = 1, widths = c(0.23, 0.5, 0.2))
#sv
...
sv3 <- ggpubr::ggarrange(pl_phenotype, pl_pgs, pl_fp, pl_gw, nrow = 1, widths = c(0.2, 0.48, 0.2, 0.22))
sv3
#sv <- ggarrange(pl_phenotype, pl_pgs, pl_fp, nrow = 1, widths = c(0.23, 0.5, 0.2))
...
sv4 <- ggpubr::ggarrange(pl_phenotype, pl_pgs, pl_fp, pl_gw, nrow = 1, widths = c(0.2, 0.48, 0.2, 0.22))
sv4
#sv <- ggarrange(pl_phenotype, pl_pgs, pl_fp, nrow = 1, widths = c(0.23, 0.5, 0.2))
#ggsave("~/Desktop/DD.png", sv, height = 2.5, width = 11)
#ggsave("~/Desktop/DD.png", pl_fp, height = 4, width = 4)
```
