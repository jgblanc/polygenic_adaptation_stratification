...
df_marg$diff <- as.numeric(df_marg$diff)
df_joint$diff <- as.numeric(df_joint$diff)
dfC1_plot <- df_C1 %>% select(-c("env", "case")) %>% melt()


ggplot(data = df_marg, aes(x = diff, y = fp, color = type)) + facet_wrap(~case) + geom_point() + geom_hline(yintercept = 0.05, color = "red") +theme_classic(base_size = 10) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1), axis.title = element_text(size = 12)) + ylim(-0.1,1.1) + geom_errorbar(aes(ymin = lowerci, ymax = upperci), width = 0.01, size =0.5)

ggplot(data = df_joint, aes(x = diff, y = fp, color = type)) + facet_wrap(~case) + geom_point() + geom_hline(yintercept = 0.05, color = "red") +theme_classic(base_size = 10) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1), axis.title = element_text(size = 12)) + ylim(-0.1,1.1) + geom_errorbar(aes(ymin = lowerci, ymax = upperci), width = 0.01, size =0.5)
```
