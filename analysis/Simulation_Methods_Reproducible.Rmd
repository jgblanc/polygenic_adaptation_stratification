...
```{r,eval = TRUE, echo=FALSE, warning=FALSE}
df <- fread("../code/Make_Plots/A_4PopSplit_shift_GXT.txt.gz")
...
df <- fread("../code/Make_Plots/A_4PopSplit_shifth23_GXT.txt.gz")
...
df <- fread("../code/Make_Plots/A_4PopSplit_shift23_TS_GXT.txt.gz")
ts <- fread("../code/Make_Plots/A_4PopSplit_shift23_TS_magnitude.txt.gz")
df <- full_join(ts, df)
...
data_nc <- df %>% group_by(envs, ts, type) %>% summarise(fp_strat = sum(`P.EN` < 0.05)/ n(), lowerci = fp_strat - 1.96 * sqrt((fp_strat * (1-fp_strat))/ n()), upperci = fp_strat + 1.96 * sqrt((fp_strat * (1-fp_strat))/ n())) %>% separate(ts, c("ts", "prob"), "-") %>% separate(envs, c("env", "diff"), "_") + scale_colour_manual(name = "", values = c("darkgreen", "navy", "darkgreen", "red"))
...
fp <- fread("../code/Make_Plots/SimpleGrid_fp_C_GXT_shift.txt.gz")
pgs <- fread("../code/Make_Plots/SimpleGrid_pgs_C_GXT_shift.txt.gz")
phenos <- fread("../code/Make_Plots/SimpleGrid_phenos_C_shift.txt.gz")
