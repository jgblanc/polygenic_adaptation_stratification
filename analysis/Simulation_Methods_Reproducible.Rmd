...
```{r,eval = TRUE, echo=FALSE, warning=FALSE}
df <- fread("../code/Make_Plots/A_4PopSplit_h20_GXT.txt.gz")
...
df <- fread("../code/Make_Plots/A_4PopSplit_h23_GXT.txt.gz")
...
make_fp_plot <- function(df, pheno_type, env_type, test_type, longitude=F) {
  ...
  if (longitude == F) {
    dat <- df %>% filter(test == test_type & pheno == pheno_type & envs == env_type) %>% group_by(type) %>% summarise(fp_strat = sum(P.EN <= 0.05)/ n(), lowerci = fp_strat - 1.96 * sqrt((fp_strat * (1-fp_strat))/ n()), upperci = fp_strat + 1.96 * sqrt((fp_strat * (1-fp_strat))/ n())) %>% filter(type %in% c("nc", "nc-Tm", "nc-ID")) %>% mutate(test = test_type) %>% mutate(type = factor(type, levels = c("nc", "nc-Tm", "nc-ID")))
    
  pl <-ggplot(data = dat, aes(x = type, y = fp_strat, fill = type)) + geom_col()+ geom_point(fill = "black", size = 3)+ xlab("") + ylab("False Postive Rate") + theme_classic(base_size = 12) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1), axis.text.x = element_text(size = 8, color = "black",  angle = 35, hjust=1), legend.position = "none") + ylim(-0.01,1) + labs(colour="GWAS") + theme(legend.text.align = 0)+ geom_hline(yintercept = 0.05, color = "red") + geom_errorbar(aes(ymin = lowerci, ymax = upperci), width =0.25) + scale_fill_manual(values = c("goldenrod2","navy", "darkgreen")) + scale_x_discrete(labels = unname(TeX(c("No covariate", "Including $T^{GWAS}", "Including confounder")))) + facet_wrap(~test)
  } else {
...
fp <- fread("../code/Make_Plots/SimpleGrid_fp_C_GXT.txt.gz")
pgs <- fread("../code/Make_Plots/SimpleGrid_pgs_C_GXT.txt.gz")
phenos <- fread("../code/Make_Plots/SimpleGrid_phenos_C.txt.gz")
gwas_weights <- fread("../code/Make_Plots/SimpleGrid_gw_C_GXT.txt.gz")
```
...
pl_pgs <- make_pgs_plot(pgs, pheno_type = "LAT", env_type = "env-1.0", test_type = "LAT", -0.25, 0.25)
...
```{r, eval = TRUE, echo=FALSE, warning=FALSE,fig.width=10, fig.height=3}
pl_fp <- make_fp_plot(fp, pheno_type = "DIAG", env_type = "env-5.0", test_type = "LAT", longitude = T)
pl_pgs <- make_pgs_plot(pgs, pheno_type = "DIAG", env_type = "env-5.0", test_type = "LAT", -0.6, 0.6)
pl_phenotype <- make_phenotype_plot(phenos, pheno_type = "DIAG", env_type = "env-5.0")
pl_gw <- make_gwas_weights_plot(gwas_weights, "LAT")
...
```{r, eval = TRUE, echo=FALSE, warning=FALSE,fig.width=10, fig.height=3}
pl_fp <- make_fp_plot(fp, pheno_type = "PS", env_type = "env-10.0", test_type = "LAT", longitude = F)
pl_pgs <- make_pgs_plot(pgs, pheno_type = "PS", env_type = "env-10.0", test_type = "LAT", -0.5, 0.5)
pl_phenotype <- make_phenotype_plot(phenos, pheno_type = "PS", env_type = "env-10.0")
pl_gw <- make_gwas_weights_plot(gwas_weights, "LAT")
...
```{r, eval = TRUE, echo=FALSE, warning=FALSE,fig.width=10, fig.height=3}
pl_fp <- make_fp_plot(fp, pheno_type = "PS", env_type = "env-10.0", test_type = "PS", longitude = F)
pl_pgs <- make_pgs_plot(pgs, pheno_type = "PS", env_type = "env-10.0", test_type = "PS", -0.5, 0.5)
pl_phenotype <- make_phenotype_plot(phenos, pheno_type = "PS", env_type = "env-10.0")
